...
var startAd;
var nextAtAd;
function startAdvertismentTimeout() {
  if (!startAd) {
  ...
  if (!loggedIn) {
    bot.chat(config.antiAfkMessage);

    adTimeout = setTimeout(startAdvertismentTimeout(), startAd - new Date().getTime());
  }
}

function stopAdTimeout() {
  startAd = null;
  nextAtAd = null;
